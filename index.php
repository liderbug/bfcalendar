...
function selcat(val) {
  var val;
  var  url = `[yourwebsite]webcal/index.php?setcat=${val}`;
...
function do_month ( $ots, $x, $mo, $yr)
{
  $ts = strtotime ("$x", mktime(0,0,0,$mo,1,$yr)); 
  $day = date ('d', $ts);
  if ($day == 22)
    $day -= 7;
...
function do_week($x, $dow, $rmo, $ryr)
{
    ...
    $wom  = $name[$x];
    $ts = date('j', strtotime("$wom $dow of $rmo $ryr"));
    return ($ts);
}
...
# ----- main () -----
$Home = "[yourhost.com]/webcal"; # set this to "yourhost.com/[pwd]"
...
echo "<table border=0 width=95% align=center>
  <tr height=100px>
    ...
    <td align=center >NEW CALENDAR<H2><br><b>$m2</b></H2>";
        ...
        switch ($i[0]) {
            ...
            case 'm':
                  ...
                  if ( $key > 0 )
                  {
                    ...
                    if ($key == $month)
                    {
                      $ret = do_month ( $r[1], $i[1], $month, $year);
                      $t1 = explode (',', $ret);
                      $H = $t1[1];
                      array_push($carray[$t1[1]], "$H|$r[4]|$r[1]|$r[3]|$r[5]|$r[0]");
                    }
                    $x=99;
                  } else {
                     $x=99;
...
for ($n = 0; $n <= $last; $n++)
    sort($carray[$n]);
#echo "<pre>"; print_r($carray);
echo "<table 3 border=1 width=100%><tr height=100px><th>Sunday</th><th>Monday</th><th>Tuesday</th><th>Wednesday</th><th>Thursday</th><th>Friday</th><th>Saturday</th></tr>\n";
foreach ($week as $w) {
    $w = trim($w);
